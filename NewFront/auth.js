...
class AuthService {
    ...
    redirectByAccountType(accountType) {
        const basePath = this.getRelativePathToRoot();
        const routes = {
            'empresa': `${basePath}empresa/empresa-dashboard.html`,
            'freelancer': `${basePath}freelancer/freelancer-dashboard.html`,
            'cliente': `${basePath}shared/marketplace.html` // Marketplace é a página principal para clientes
        };

        const route = routes[accountType] || `${basePath}shared/marketplace.html`;
    ...
    protectRoute(requiredAccountType = null) {
        if (!this.isAuthenticated()) {
            const basePath = this.getRelativePathToRoot();
            window.location.href = `${basePath}shared/cadastro.html`;
            return false;
        }

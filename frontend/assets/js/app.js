...
const DevHubAuth = {
  register: async (e) => {
    ...
    try {
      ...
      if (response.status === 'ok') {
        ...
        localStorage.setItem('devhubCurrentUser', JSON.stringify(usuarioSemSenha));

        // Redirecionar baseado no tipo de usuário
        setTimeout(() => {
          if (tipo === 'cliente') window.location.href = 'perfil-cliente.html';
          else if (tipo === 'empresa') window.location.href = 'admin-empresa.html';
          else if (tipo === 'freelancer') window.location.href = 'admin-freelancer.html';
        }, 1500);
      }
  ...
  // Função para logout
  logout: () => {
    localStorage.removeItem('devhubCurrentUser');
  ...
  isLoggedIn: () => {
    return localStorage.getItem('devhubCurrentUser') !== null;
  },

  // Obter dados do usuário atual
  getCurrentUser: () => {
    const userData = localStorage.getItem('devhubCurrentUser');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Tornar o campo de busca funcional (Enter e digitação)
  ...
  const currentUser = DevHubAuth.getCurrentUser();
  if (currentUser) {
...
window.DevHubAdmin = DevHubAdmin;
